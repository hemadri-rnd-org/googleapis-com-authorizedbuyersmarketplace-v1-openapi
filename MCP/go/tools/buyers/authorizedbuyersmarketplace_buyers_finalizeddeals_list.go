...
func Authorizedbuyersmarketplace_buyers_finalizeddeals_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/%s/finalizedDeals%s", cfg.BaseURL, parent, queryString)
		...
		var result models.ListFinalizedDealsResponse
...
func CreateAuthorizedbuyersmarketplace_buyers_finalizeddeals_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_parent_finalizedDeals",
		mcp.WithDescription("Lists finalized deals. Use the URL path "/v1/buyers/{accountId}/finalizedDeals" to list finalized deals for the current buyer and its clients. Bidders can use the URL path "/v1/bidders/{accountId}/finalizedDeals" to list finalized deals for the bidder, its buyers and all their clients."),
		mcp.WithString("parent", mcp.Required(), mcp.Description("Required. The buyer to list the finalized deals for, in the format: `buyers/{accountId}`. When used to list finalized deals for a bidder, its buyers and clients, in the format `bidders/{accountId}`.")),
		...
		mcp.WithString("pageToken", mcp.Description("The page token as returned from ListFinalizedDealsResponse.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Authorizedbuyersmarketplace_buyers_finalizeddeals_listHandler(cfg),
	}
}
